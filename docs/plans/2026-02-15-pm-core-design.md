  ...
  "hooks": [
    {
      "event": "Stop",
      "type": "prompt",
      "prompt": "Check git diff for this session. If any of these changed: module structure, new shared utilities, DI patterns, naming conventions, CLAUDE.md, AGENTS.md — suggest running /pm:upgrade. If nothing architectural changed, say nothing."
    }
  ]
}
```
...
- **CCPM** (automazeio/ccpm): conductor pattern, context isolation, frontmatter
  state machine, GitHub sync, parallel agent orchestration
